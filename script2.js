...
(function () {
    ...
    function getHistory(key) {
        if (!historyStacks[key]) {
            historyStacks[key] = { undo: [], redo: [] };
        }
        return historyStacks[key];
    }

    function pushHistory() {
        const key = `pixono_diary_${currentTheme}_p${currentPage}`;
    ...
    function getCurrentState() {
        ...
        document.querySelectorAll('.diary-sticker').forEach(s => {
            ...
            let content = imgSpan ? imgSpan.src : (emojiSpan ? emojiSpan.textContent : "");
            stickers.push({
                content: content,
                isImage: !!imgSpan,
                left: s.style.left,
        ...
        const texts = [];
        document.querySelectorAll('.diary-text-box').forEach(t => {
            const content = t.querySelector('.text-content')?.innerText || "";
            texts.push({
                content: content,
                left: t.style.left,
                top: t.style.top,
                ...
                zIndex: t.style.zIndex || 15
            });
        });

        return {
            leftText: leftArea?.value || '',
            rightText: rightArea?.value || '',
    ...
    function restoreState(state) {
        ...
        const leftArea = document.getElementById('leftPageText');
        const rightArea = document.getElementById('rightPageText');
        if (leftArea) leftArea.value = state.leftText || '';
        if (rightArea) rightArea.value = state.rightText || '';
        if (state.fontFamily) {
            const fs = document.getElementById('fontSelector');
            if (fs) fs.value = state.fontFamily;
            if (leftArea) leftArea.style.fontFamily = state.fontFamily;
            if (rightArea) rightArea.style.fontFamily = state.fontFamily;
        }

        clearPageDecoration();
        state.stickers?.forEach(s => addSticker(s.content, s.left, s.top, s.scale, s.rotation, s.isImage, s.zIndex));
        state.texts?.forEach(t => addTextBox(t.content, t.left, t.top, t.width, t.height, t.zIndex));
    }

    function clearPageDecoration() {
        document.querySelectorAll('.diary-sticker').forEach(s => s.remove());
        document.querySelectorAll('.diary-text-box').forEach(t => t.remove());
        const tb = document.getElementById('stickerToolbar');
        if (tb) tb.classList.remove('active');
    }

    window._pixonoChooseDiary = function(style) {
        try { saveDiaryPage(); } catch(e) {} // Save current before theme change
    ...
    function setupViewerInteractions() {
        ...
        writeBtn?.addEventListener('click', () => {
            ...
            if (area) {
                area.classList.remove('decorate-mode');
                area.style.pointerEvents = 'none'; // Allow textarea clicks
            }
            playBubble();
        });
        ...
        decorateBtn?.addEventListener('click', () => {
            ...
            const area = document.getElementById('stickerArea');
            if (area) {
                area.classList.add('decorate-mode');
                area.style.pointerEvents = 'auto'; // Enable for sticker/text addition
            }
            playBubble();
        });
        ...
        document.getElementById('undoBtn')?.addEventListener('click', () => {
            ...
            if (hist.undo.length > 0) {
                hist.redo.push(JSON.stringify(getCurrentState()));
                const last = hist.undo.pop();
                restoreState(last);
                playPop();
            }
        });
        ...
        document.getElementById('redoBtn')?.addEventListener('click', () => {
            ...
            if (hist.redo.length > 0) {
                hist.undo.push(JSON.stringify(getCurrentState()));
                const nextS = hist.redo.pop();
                restoreState(nextS);
                playPop();
            }
        });
        ...
        document.getElementById('resetBtn')?.addEventListener('click', () => {
            if (confirm("Reset current page decoration and text?")) {
                pushHistory();
                clearPageDecoration();
                const leftArea = document.getElementById('leftPageText');
                const rightArea = document.getElementById('rightPageText');
                if (leftArea) leftArea.value = '';
                if (rightArea) rightArea.value = '';
                playBubble();
            }
        });
        ...
        const area = document.getElementById('stickerArea');
        if (area) {
            // Enable pointer events on area to catch clicks on empty space
            ...
            area.addEventListener('mousedown', (e) => {
                if (e.target === area) {
                    ...
                    // Do not place text in the top margins (toolbar area)
                    if (clickY < 50) return;

                    // Detect spine area (dead zone for text creation)
                    const centerX = rect.width / 2;
                    if (clickX > centerX - 25 && clickX < centerX + 25) return;

                    // Positioning (centering the 100px wide box)
                    const posX = clickX - 50; 
                    const posY = clickY - 15;
    ...
    function addTextBox(content = "", x = 100, y = 100, width = "auto", height = "auto", zIndex = 15) {
        const area = document.getElementById('stickerArea');
        if (!area) return;

        const box = document.createElement('div');
        box.className = 'diary-text-box';
        box.style.left = typeof x === 'number' ? `${x}px` : x;
        ...
        box.innerHTML = `
            <div class="text-content" contenteditable="true" spellcheck="false">${content}</div>
            <div class="sticker-handle handle-delete" title="Delete">✕</div>
            <div class="sticker-handle handle-resize" title="Resize">⤡</div>
        `;

        const contentEl = box.querySelector('.text-content');
        contentEl.addEventListener('focus', () => selectSticker(box));
        contentEl.addEventListener('blur', () => {
            saveDiaryPage(); // Auto save on text change
        });
        ...
        box.addEventListener('mousedown', (e) => {
            ...
            if (e.target.closest('.handle-resize')) {
                // simple resize for text box
                startResize(e, box);
                return;
            }
            if (e.target === box || (e.target !== contentEl && !e.target.closest('.sticker-handle'))) {
                startDrag(e, box);
            }
        });
    ...
    function loadDiaryPage() {
        ...
        if (data) {
            ...
            if (leftArea) leftArea.value = '';
            if (rightArea) rightArea.value = '';
            if (leftArea) leftArea.style.fontFamily = "'Quicksand', sans-serif";
            if (rightArea) rightArea.style.fontFamily = "'Quicksand', sans-serif";
        }
    }
    ...
    function setupScrollAnimations() {
        ...
        if (startBtn && scroller) {
            startBtn.addEventListener('click', (e) => {
                e.preventDefault();
                playPop();
                // scrollIntoView can sometimes be finicky with scroll-snap,
                // using scrollTo is often more robust for simple transitions.
                scroller.scrollTo({
    ...
    window.addEventListener('mousemove', (e) => {
        ...
        // Diary Tilt Effect
        const bookRoot = document.querySelector('.book-container');
        const book = document.getElementById('diaryBook');
        if (bookRoot && book) {
            ...
            // Extremely subtle tilt (max 1.2 degrees)
            const tiltX = (e.clientY - centerY) / (window.innerHeight / 2) * -1.2;
            const tiltY = (e.clientX - centerX) / (window.innerWidth / 2) * 1.2;
            
            // Apply to the layout for visual tilt
            book.style.transform = `rotateX(${tiltX}deg) rotateY(${tiltY}deg)`;
            // Also apply a slight parallax to the sticker area if we want, 
